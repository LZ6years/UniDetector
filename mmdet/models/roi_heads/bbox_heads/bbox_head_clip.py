...
class BBoxHeadCLIP(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(BBoxHeadCLIP, self).__init__(init_cfg)
        ...
        if self.cat_freq_path is not None:
            self.freq_weight = self.load_class_freq()
        
        self.attnpool = AttentionPool2d(7, 64*32, 32, 1024)
        #self.attnpool = AttentionPool2d(9, 80*32, 40, 640)
    ...
    def build_zs_layer(self):
        #zs_weights = torch.load(self.zeroshot_path)
        zs_weights = np.load(self.zeroshot_path)
        zs_weights = torch.tensor(zs_weights, dtype=torch.float32).permute(1, 0).contiguous()
        zs_weights = torch.cat([zs_weights, zs_weights.new_zeros((zs_weights.shape[0], 1))],  dim=1)
        self.register_buffer('zs_weights', zs_weights)
    
    
    def load_class_freq(self):
        cat_info = json.load(open(self.cat_freq_path, 'r'))
        cat_info = torch.tensor([c['image_count'] for c in sorted(cat_info, key=lambda x: x['id'])] + [1.0], device='cuda')
        freq_weight = cat_info.float()
        return freq_weight
    ...
    def forward(self, x):
        x = self.attnpool(x)
        x_cls = x
        cls_score = torch.mm(100 * F.normalize(x_cls, p=2, dim=1), F.normalize(self.zs_weights, p=2,dim=0))
            ...
            if cls_score.numel() > 0:
                ...
                if len(appeared) < self.fed_loss_num_cat:
                    ...
                    if self.freq_weight is not None:
                        prob[self.freq_weight==0] = 0
                    more_appeared = torch.multinomial(prob, max(self.fed_loss_num_cat - len(appeared), 0), replacement=False)
                ...
                loss_cls_ = F.binary_cross_entropy_with_logits(cls_score[:, :-1], target, reduction='none') # B x C
                loss_cls_ = torch.sum(loss_cls_ * weight) / B * self.loss_cls.loss_weight
...
@HEADS.register_module()
class BBoxHeadCLIPInference(BBoxHeadCLIP):
    """Simplest RoI head, with only two fc layers for classification and
    regression respectively."""
