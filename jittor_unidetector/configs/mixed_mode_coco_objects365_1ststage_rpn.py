...
model = dict(
    type='FasterRCNN',
    backbone=dict(
        type='ResNet',  # 保持与原始版本一致，使用ResNet
        depth=50,
        num_stages=4,
        out_indices=(0, 1, 2, 3),
        frozen_stages=1,
...
optimizer = dict(type='SGD', lr=0.005, momentum=0.9, weight_decay=0.0001)  # 降低学习率
optimizer_config = dict(grad_clip=dict(max_norm=20, norm_type=2))


lr_config = dict(
    type='MultiStepLR',  # 改为MultiStepLR来处理step列表
    milestones=[3, 4],  # 在第3和第4个epoch降低学习率
    gamma=0.1)  # 每次降低到0.1倍
# total_epochs = 4
runner = dict(
